...
use super::stats::ProcessStatistics;
use super::utils::{get_num_records, write_fastq_to_buffer_set};
use crate::cli::{FastqOutput, FilterOptions};
        ...
        let handle = std::thread::spawn(move || -> Result<ProcessStatistics> {
            ...
            for (idx, record) in reader.into_range_iter(start as i64, stop)?.enumerate() {
                ...
                for segment in record.into_iter() {
                    ...
                    write_fastq_to_buffer_set(&mut local_buffers, &segment)?;
                    if counts.len() == 1 {
                        counts[0] += 1;
                    } else {
