...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let schema = unsafe {
        ...
        let rc = VDBManagerMakeSchema(mgr.0, &mut schema);
        if rc != 0 {
    ...
    let table = unsafe {
        ...
        let rc = VDBManagerOpenTableRead(mgr.0, &mut table, schema.0, path.as_ptr());
        if rc != 0 {
    ...
    let cursor = unsafe {
        ...
        let rc = VTableCreateCachedCursorRead(table.0, &mut cursor, BUFFER_SIZE);
        if rc != 0 {
    ...
    unsafe {
        ...
        match is_column_present(table, "READ") {
            Ok(true) => {
                ...
                let rc = VCursorAddColumn(cursor.0, &mut indices.seq, col_name.as_ptr());
                if rc != 0 {
        ...
        match is_column_present(table, "QUALITY") {
            Ok(true) => {
                ...
                let rc = VCursorAddColumn(cursor.0, &mut indices.qual, col_name.as_ptr());
                if rc != 0 {
        ...
        match is_column_present(table, "READ_START") {
            Ok(true) => {
                ...
                let rc = VCursorAddColumn(cursor.0, &mut indices.read_start, col_name.as_ptr());
                if rc != 0 {
        ...
        match is_column_present(table, "READ_LEN") {
            Ok(true) => {
                ...
                let rc = VCursorAddColumn(cursor.0, &mut indices.read_len, col_name.as_ptr());
                if rc != 0 {
        ...
        match is_column_present(table, "READ_TYPE") {
            Ok(true) => {
                ...
                let rc = VCursorAddColumn(cursor.0, &mut indices.read_type, col_name.as_ptr());
                if rc != 0 {
...
fn process_rows(
    ...
    unsafe {
        ...
        let mut row_count = 0;
        let rc = VCursorIdRange(cursor.0, indices.seq, &mut first_row_id, &mut row_count);
        if rc != 0 {
